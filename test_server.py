# -*- coding: utf-8 -*-
"""
Created on Mon Nov 18 12:03:34 2019
...
class UserManager: # 사용자관리 및 채팅 메세지 전송을 담당하는 클래스
   ...
   def addUser(self, username, conn, addr): # 사용자 ID를 self.users에 추가하는 함수
      ...
      self.sendMessageToAll('[%s]PC 접속.' %username)
      print('+++ 접속 클라이언트 수 [%d]' %len(self.users))
      return username
   ...
   def removeUser(self, username): #사용자를 제거하는 함수
      ...
      self.sendMessageToAll('[%s]PC 접속.' %username)
      print('--- 접속 클라이언트 수 수 [%d]' %len(self.users))
...
class MyTcpHandler(socketserver.BaseRequestHandler):
   ...
   def registerUsername(self):
      while True:
         username = str(self.client_address[1])
